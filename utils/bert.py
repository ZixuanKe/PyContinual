...
def lookfor_baseline_variable(self, args):
    ...
    self.tsv_para = \
        ['module.model.bert.' + coder + '.layer.' + str(
        ...
        ['module.model.bert.encoder.layer.' + str(
    ...
    for layer_id in range(12):
        ...
        for c_t in range(args.ntasks):
            if n == 'module.model.bert.encoder.layer.' + str(
                ...
                c_t) + '.weight':
                return \
                    model_ori.model.bert.encoder.layer[
                        layer_id].output.adapters.adapter.adapter_down.linear_down.adapters.capsule_net.tsv_capsules.tsv[
                        t][
                        c_t].data
            elif n == 'module.model.bert.encoder.layer.' + str(
                    layer_id) + '.output.adapters.adapter.adapter_down.linear_down.adapters.capsule_net.semantic_capsules.fc1.' + str(
                c_t) + '.bias':
                return \
                    model_ori.model.bert.encoder.layer[
...
def mask(model, accelerator, args):
    ...
    for layer_id in range(model_ori.config.num_hidden_layers):
        if 'adapter_hat' in args.baseline or 'adapter_cat' in args.baseline \
            ...
            fc1_key = 'module.model.bert.encoder.layer.' + str(
                layer_id) + '.output.adapters.adapter.adapter_down.linear_down.adapters.fc1'  # gfc1
            fc2_key = 'module.model.bert.encoder.layer.' + str(
                layer_id) + '.output.adapters.adapter.adapter_down.linear_down.adapters.fc2'  # gfc2

            masks[fc1_key], masks[fc2_key] = model_ori.model.bert.encoder.layer[
                layer_id].output.adapters.adapter.adapter_down.linear_down.adapters.mask()

            if 'adapter_cat' in args.baseline:
                cat_masks[fc1_key], cat_masks[fc2_key] = model_ori.model.bert.encoder.layer[
                    layer_id].output.adapters.adapter.adapter_down.linear_down.adapters.cat_mask()
                masks[fc1_key], masks[fc2_key] = torch.min(masks[fc1_key], cat_masks[fc1_key]), torch.min(
                    masks[fc2_key], cat_masks[fc2_key])
            ...
            if 'adapter_cat' in args.baseline:
                cat_masks[fc1_key], cat_masks[fc2_key] = model_ori.model.bert.encoder.layer[
                    layer_id].output.adapters.adapter.adapter_up.adapters.cat_mask()
                masks[fc1_key], masks[fc2_key] = torch.min(masks[fc1_key], cat_masks[fc1_key]), torch.min(
                    masks[fc2_key], cat_masks[fc2_key])
