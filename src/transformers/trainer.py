...
class Trainer:
    ...
    def __init__(
        ...
        t=None,mask_back=None,mask_pre=None,model_old=None,fisher=None,
        tsv_para=None,
    ):

        self.t = t
        self.mask_back=mask_back
        self.tsv_para = tsv_para
        self.mask_pre=mask_pre
        self.thres_emb = transformer_args.thres_emb
        self.thres_cosh = transformer_args.thres_cosh
        self.lamb=transformer_args.lamb
        self.smax=transformer_args.smax
        self.clipgrad=transformer_args.clipgrad
    ...
    def create_optimizer_and_scheduler(self, num_training_steps: int):
        ...
        if self.optimizer is None:
            ...
            optimizer_grouped_parameters = [
                {   'name': [n for n, p in self.model.named_parameters() if not any(nd in n for nd in no_decay)],
                    "params": [p for n, p in self.model.named_parameters() if not any(nd in n for nd in no_decay)],
                    "weight_decay": self.args.weight_decay,
                },
                {   "name": [n for n, p in self.model.named_parameters() if any(nd in n for nd in no_decay)],
            ...
            for step, inputs in enumerate(epoch_iterator):
                ...
                if ((step + 1) % self.args.gradient_accumulation_steps != 0) and self.args.local_rank != -1:
                    ...
                    with model.no_sync():
                        ...
                        if transformer_args.task_mask:
                            s=(self.smax-1/self.smax)*step/len(epoch_iterator)+1/self.smax #len(epoch_iterators) -> B
                            tr_loss += self.training_step(model, inputs,s)
                        else:
                            tr_loss += self.training_step(model, inputs)
                else:
                    if transformer_args.task_mask:
                        s=(self.smax-1/self.smax)*step/len(epoch_iterator)+1/self.smax
                        # print('s: ',s)
                ...
                if (step + 1) % self.args.gradient_accumulation_steps == 0 or (
                    ...
                    if is_torch_tpu_available():
                        ...
                        self.scaler.step(self.optimizer,t=self.t,mask_back=self.mask_back,
                                         tsv_para=self.tsv_para,model=model)
                        self.scaler.update()
                    else:
                        self.optimizer.step(t=self.t,mask_back=self.mask_back,
                                            tsv_para=self.tsv_para,model=model)
                    ...
                    if transformer_args.task_mask:
                        ...
                        for n,p in model.named_parameters():
                            if 'adapter_mask.e' in n or 'adapter_capsule_mask.e' in n or 'tsv_capsules.e' in n:
    ...
    def training_step(self, model: nn.Module, inputs: Dict[str, Union[torch.Tensor, Any]],s=None) -> torch.Tensor:
        """
        ...
        if self.use_amp:
            with autocast():
                if transformer_args.task_mask:
                    loss = self.compute_loss(model, inputs,s)
                else:
        ...
        if transformer_args.task_mask:
            ...
            if self.t>0:
                for n,p in model.named_parameters():
                    ...
                    elif (self.tsv_para is not None) and (n in self.tsv_para) and p.grad is not None:
                        p.grad.data*=self.model.get_view_for_tsv(n,self.t) #open for general

                    # elif p.grad is None:
                    #     print('n: ',n)

            # with this no training can happen
            # Compensate embedding gradients
            # for n,p in model.named_parameters():
            #     if 'adapter_mask.e' in n or \
            #         'adapter_capsule_mask.e' in n or \
            #         'tsv_capsules.e' in n: # we dont want etsv
