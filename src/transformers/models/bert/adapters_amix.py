...
class BertAdapterMask(BertAdapter):
    def __init__(self, config):
        ...
        if 'shared-specific' in transformer_args.mix_type:
            ...
            self.fc4=torch.nn.Linear(config.adapter_size,config.hidden_size)
        if 'Attn-HCHP-Adapter' in transformer_args.mix_type:
            ...
            n_orders = list(range(transformer_args.ntasks))
            self.n_order = [n_orders[:]]
            self.fc_zoomin.append(nn.Linear(transformer_args.bert_hidden_size, transformer_args.semantic_cap_size))
            self.fc_zoomout.append(nn.Linear(transformer_args.semantic_cap_size,transformer_args.bert_hidden_size))
            self.convs4.append(nn.Conv1d(transformer_args.semantic_cap_size, 2, 1))

            seed = transformer_args.data_seed
            for n in range(transformer_args.n_aug):
                self.fc_zoomin.append(nn.Linear(transformer_args.bert_hidden_size, transformer_args.semantic_cap_size))
                self.fc_zoomout.append(nn.Linear(transformer_args.semantic_cap_size,transformer_args.bert_hidden_size))
                self.convs4.append(nn.Conv1d(transformer_args.semantic_cap_size, 2, 1))
                # random.Random(seed).shuffle(n_orders)
                if transformer_args.nsamples > 1:
                    #you may want to only sample some: random.sample(population, k, *, counts=None)
                    n_orders_samples = random.Random(seed).sample(n_orders,transformer_args.nsamples)
    ...
    def forward(self,x,t,s,smax=400,pre_t=None,l=None,start_mixup=False,idx=None):
        #someday, we may make use of start_mixup
        #mixup here in adapter
        # print('l: ',l)
        #do we need to random choose one samples actually
        ...
        if start_mixup and pre_t is not None and pre_t=='tmix': #AP
        ...
        elif start_mixup and 'Attn-HCHP-Adapter' in transformer_args.mix_type:
            gfc1,gfc2=self.mask(t=t,s=s)
            h = self.get_feature(gfc1,gfc2,x)
            h = self.attention_feature(t,x,h,l,smax)

        elif start_mixup and 'Attn-HCHP-AMHRA' in transformer_args.mix_type :
            gfc1,gfc2=self.mask(t=t,s=s)
            h = self.get_feature(gfc1,gfc2,x)
            if l == 0: pass #the firsst augmentation is HAT it self
            else: h = self.attention_feature(t,x,h,l,smax)


        elif 'Attn-HCHP-AMHRA' in transformer_args.mix_type:
            gfc1,gfc2=self.mask(t=t,s=s)
            h = self.get_feature(gfc1,gfc2,x)
            h = self.attention_feature(t,x,h,0,smax)

        elif start_mixup and transformer_args.mix_type == 'HCHP':
            gfc1,gfc2=self.mask(t=t,s=s)
            h = self.get_feature(gfc1,gfc2,x)

            with torch.no_grad():
                gfc1,gfc2=self.mask(t=pre_t,s=smax)
                pre_h = self.get_feature(gfc1,gfc2,x)
    ...
    def attention_feature(self,t,x,h,l,smax):
        ...
        for pre_t in self.n_order[l]:
            ...
            cos_sim = self.cos(pre_h,h) #similarity
            z= cos_sim.view(batch_size,transformer_args.semantic_cap_size,1)

            decision_learner = self.convs4[l](z).squeeze(-1)

            gumbel_one_hot =  F.gumbel_softmax(decision_learner,hard=True)
            score = gumbel_one_hot[:,0]
            decision_maker.append(score.view(-1,1).clone())


        decision_maker = torch.cat(decision_maker, 1)
        pre_hs = torch.cat(pre_hs, 1)

        h = (pre_hs * decision_maker.unsqueeze(-1).unsqueeze(-1)).sum(1)
        h = self.fc_zoomout[l](h)
        return h
    ...
    def mask(self,t,s=1):
       ...
       # if torch.isinf(efc1).any() or torch.isnan(efc1).any() or torch.isinf(efc2).any() or torch.isnan(efc2).any():
       #     print('clamp efc1')
       #     clamp_value = torch.finfo(efc1.dtype).max - 1000
       #     efc1 = torch.clamp(efc1, min=-clamp_value, max=clamp_value)
       #
       #     print('clamp efc2')
       #     clamp_value = torch.finfo(efc2.dtype).max - 1000
       #     efc2 = torch.clamp(efc2, min=-clamp_value, max=clamp_value)
