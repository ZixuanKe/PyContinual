...
class BertAdapter(nn.Module):
    def __init__(self, config):
        ...
        if 'SelfAttn-HCHP' in transformer_args.mix_type :
            self.attn1 = Self_Attn(config.adapter_size)
            self.attn2 = Self_Attn(config.hidden_size)
    ...
    def forward(self,x):
        ...
        if 'SelfAttn-HCHP' in transformer_args.mix_type: h,p1 = self.attn1(h)
        h=self.activation(self.fc2(h))
        if 'SelfAttn-HCHP' in transformer_args.mix_type: h,p2 = self.attn2(h)
...
class BertAdapterMask(BertAdapter):
    def __init__(self, config):
        ...
        self.config = config
        if 'shared-specific' in config.mix_type and not self.config.is_aux: # this is for main
            self.fc3=torch.nn.Linear(config.hidden_size,config.adapter_size)
            self.fc4=torch.nn.Linear(config.adapter_size,config.hidden_size//2)
            self.fc2=torch.nn.Linear(config.adapter_size,config.hidden_size//2)
            self.efc2=torch.nn.Embedding(transformer_args.ntasks,config.hidden_size//2)

            D = transformer_args.semantic_cap_size//2
            self.Co = 100
            Ks = [3,4,5]
            self.len_ks = len(Ks)

            self.share_convs3 = nn.ModuleList([nn.Conv1d(D, self.Co, K, padding=K-2) for K in Ks])
            # self.specific_convs3 = nn.ModuleList([nn.Conv1d(D, self.Co, K, padding=K-2) for K in Ks])

            # self.specific_convs4 = nn.ModuleList()
            # for n in range(transformer_args.ntasks):
            #     self.specific_convs4.append(nn.Conv1d(self.Co*self.len_ks, 2, 1))
            self.share_convs4 = nn.Conv1d(self.Co*self.len_ks, 2, 1)
    ...
    def forward(self,x,t,s,smax=400,pre_t=None,l=None,start_mixup=False,idx=None):
        # print('pre_t: ',pre_t)
        if 'shared-specific' in self.config.mix_type and not self.config.is_aux:# this is for main
        # if 'shared-specific' in transformer_args.mix_type:

            share_h=self.activation(self.fc3(x))
            share_h=self.activation(self.fc4(share_h))


            #TODO: we want some gate to make sure, only the suitable things is being used
            #TODO: but we should not compare with current task, ensembel does not doing good

            if transformer_args.share_gate:
                feat_h = [F.relu(conv(share_h.transpose(1, 2))) for conv in self.share_convs3]  # [(N,Co,L), ...]*len(Ks)
                feat_h = [F.max_pool1d(a, a.size(2)).squeeze(2) for a in feat_h]
                feat_h = torch.cat(feat_h, 1).unsqueeze(-1)
                decision_learner = self.share_convs4(feat_h).squeeze(-1)

                share_gate = F.gumbel_softmax(decision_learner,hard=True)
                share_gate=share_gate[:,0]
                # print('share_gate: ',share_gate)
                share_h = share_h * share_gate.view(-1,1,1)#detect whether to use shared knoweldge

            gfc1,gfc2=self.mask(t=t,s=s)
            specific_h = self.get_feature(gfc1,gfc2,x)
            h = torch.cat([share_h,specific_h],dim=2)


            # specific_gates = [] #detect which specific to use
            # hs = []
            # for pre_t in range(t+1):
            #     if pre_t == t:
            #         gfc1,gfc2=self.mask(t=pre_t,s=s)
            #         h = self.get_feature(gfc1,gfc2,h)
            #     else:
            #         with torch.no_grad():
            #             gfc1,gfc2=self.mask(t=pre_t,s=s)
            #             h = self.get_feature(gfc1,gfc2,h)
            #     hs.append(h.unsqueeze(1).clone())
            #
            #     feat_h = [F.relu(conv(h.transpose(1, 2))) for conv in self.specific_convs3]  # [(N,Co,L), ...]*len(Ks)
            #     feat_h = [F.max_pool1d(a, a.size(2)).squeeze(2) for a in feat_h]
            #     feat_h = torch.cat(feat_h, 1).unsqueeze(-1)
            #     decision_learner = self.specific_convs4[pre_t](feat_h).squeeze(-1)
            #     specific_gate = F.gumbel_softmax(decision_learner,hard=True)[:,0]
            #     specific_gates.append(torch.ones_like(specific_gate.unsqueeze(-1).clone()))
            #
            # hs = torch.cat(hs,dim=1)
            # specific_gates = torch.cat(specific_gates,dim=1)
            # print('specific_gates: ',specific_gates.size())
            # print('hs: ',hs.size())
            #
            # h = (hs * specific_gates.view(-1,t+1,1,1)).sum(1)
...
class BertAdapterUcl(nn.Module):
    def __init__(self, config):
        ...
        self.fc1=BayesianLinear(config.hidden_size, transformer_args.adapter_size,ratio=transformer_args.ratio)
        self.fc2 = BayesianLinear(transformer_args.adapter_size, config.hidden_size,ratio=transformer_args.ratio)
...
class BertAdapterOwm(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.relu = torch.nn.ReLU()
        self.fc4=torch.nn.Linear(config.hidden_size*transformer_args.max_seq_length,transformer_args.adapter_size)
        #TODO: self.fc4=torch.nn.Linear(config.hidden_size,transformer_args.adapter_size)
        self.fc1=torch.nn.Linear(transformer_args.adapter_size,transformer_args.adapter_size)
        self.fc2=torch.nn.Linear(transformer_args.adapter_size,transformer_args.adapter_size)
        self.fc3=torch.nn.Linear(transformer_args.adapter_size,transformer_args.max_seq_length*config.hidden_size)
...
class BertAdapterTwoModules(BertAdapterMask):
    ...
    def forward(self,x,t,s,smax=400):
        ...
        # shared
        shared_h=self.relu(self.shared_fc1(x))
        shared_h=self.relu(self.shared_fc2(shared_h))
        ...
        if t > 0:
            for pre_t in range(t):
                ...
                pre_h=self.relu(self.fc1(x)).data.detach()
                pre_h=pre_h*pre_gfc1.expand_as(pre_h).data.detach()

                pre_h=self.relu(self.fc2(pre_h)).data.detach()
...
class BertAdapterCapsuleGrow(BertAdapter):
    ...
    def forward(self,x,t):
        ...
        batch_size = x.size(0)
        h=self.relu(self.fc1[t](x))
        h=self.relu(self.fc2[t](h))
        ...
        with torch.no_grad():
            for pre_t in range(t):
                pre_h=self.relu(self.fc1[pre_t](x)).detach()
                pre_h=self.relu(self.fc2[pre_t](pre_h)).detach()
