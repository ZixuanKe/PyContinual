...
def embedding_generation(args):
    ...
    for dataset in datasets:
        for file_name in os.listdir(dataset):
            with open(dataset+'/'+file_name) as asc_file:
                if 'json' in file_name and 'squad' not in file_name:
                    lines = json.load(asc_file)
                    for (i, ids) in enumerate(lines):
                        asc_str.append(lines[ids]['sentence'])
    ...
    if not path.exists("./dat/dsc/w2v_embedding"):
        embeddings_index = {}
        ...
        torch.save(embeddings,'./dat/dsc/w2v_embedding')
        torch.save(vocab_size,'./dat/dsc/vocab_size')
    else:
        embeddings = torch.load('./dat/dsc/w2v_embedding')
        vocab_size = torch.load('./dat/dsc/vocab_size')
...
def get(logger=None,args=None):
    ...
    for t in range(args.ntasks):
        ...
        #TSo that no change on data even if we change the seed
        if not os.path.exists("./dat/dsc/"+str(random_sep[t])+"/w2v_train_data_"+str(args.train_data_size)+'_'+str(args.data_seed)):
            torch.save(train_data,"./dat/dsc/"+str(random_sep[t])+"/w2v_train_data_"+str(args.train_data_size)+'_'+str(args.data_seed))
            logger.info("  save")
        else: #in that case, change seed will not change data
            train_data = torch.load("./dat/dsc/"+str(random_sep[t])+"/w2v_train_data_"+str(args.train_data_size)+'_'+str(0)) #always use 0
            logger.info("  load")
        ...
        #     valid_examples = valid_examples[:args.dev_data_size] #TODO: remove
        ...
        eval_features = \
            data_utils.convert_examples_to_features_w2v_dsc\
