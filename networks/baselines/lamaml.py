...
class Learner:
    ...
    def functional_bartforconditionalgeneration(
        ...
        lm_logits = F.linear(outputs[0], weight=fast_weights['model.shared.weight'])
        masked_lm_loss = None
...
def functional_bart_attention(fast_weights, config, hidden_states, layer_idx, attention_type, key_value_states=None, past_key_value=None, attention_mask=None, layer_head_mask=None, is_train=True):
    ...
    if attention_type == 'encoder':
        query_states = F.linear(hidden_states, 
            fast_weights['model.encoder.layers.'+str(layer_idx)+'.self_attn.q_proj.weight'], 
            fast_weights['model.encoder.layers.'+str(layer_idx)+'.self_attn.q_proj.bias']
        ) * scaling
    elif attention_type == 'decoder':
        query_states = F.linear(hidden_states,
        fast_weights['model.decoder.layers.'+str(layer_idx)+'.self_attn.q_proj.weight'],
        fast_weights['model.decoder.layers.'+str(layer_idx)+'.self_attn.q_proj.bias']
    ) * scaling
    elif attention_type == 'cross':
        query_states = F.linear(hidden_states,
        fast_weights['model.decoder.layers.'+str(layer_idx)+'.encoder_attn.q_proj.weight'],
        fast_weights['model.decoder.layers.'+str(layer_idx)+'.encoder_attn.q_proj.bias']
    ) * scaling
    ...
    if (is_cross_attention and past_key_value is not None and past_key_value[0].shape[2] == key_value_states.shape[1]):
        ...
        key_states = _shape(F.linear(key_value_states,
                              fast_weights['model.decoder.layers.'+str(layer_idx)+'.encoder_attn.k_proj.weight'],
        ...
        value_states = _shape(F.linear(key_value_states,
                              fast_weights['model.decoder.layers.'+str(layer_idx)+'.encoder_attn.v_proj.weight'],
        ...
        if attention_type == 'encoder':
            key_states = F.linear(hidden_states, 
                                  fast_weights['model.encoder.layers.'+str(layer_idx)+'self_attn.k_proj.weight'], 
                                  fast_weights['model.encoder.layers.'+str(layer_idx)+'self_attn.k_proj.bias']
                                )
            value_states = F.linear(hidden_states, 
                                  fast_weights['model.encoder.layers.'+str(layer_idx)+'self_attn.v_proj.weight'], 
                                  fast_weights['model.encoder.layers.'+str(layer_idx)+'self_attn.v_proj.bias']
                                )
        elif attention_type == 'decoder':
            key_states = F.linear(hidden_states,
                                    fast_weights['model.decoder.layers.'+str(layer_idx)+'.self_attn.k_proj.weight'],
                                    fast_weights['model.decoder.layers.'+str(layer_idx)+'.self_attn.k_proj.bias']
                                )
            value_states = F.linear(hidden_states,
                                    fast_weights['model.decoder.layers.'+str(layer_idx)+'.self_attn.v_proj.weight'],
                                    fast_weights['model.decoder.layers.'+str(layer_idx)+'.self_attn.v_proj.bias']
                                )
        elif attention_type == 'cross':
            key_states = F.linear(hidden_states,
                                    fast_weights['model.decoder.layers.'+str(layer_idx)+'.encoder_attn.k_proj.weight'],
                                    fast_weights['model.decoder.layers.'+str(layer_idx)+'.encoder_attn.k_proj.bias']
                                )
            value_states = F.linear(hidden_states,
                                    fast_weights['model.decoder.layers.'+str(layer_idx)+'.encoder_attn.v_proj.weight'],
                                    fast_weights['model.decoder.layers.'+str(layer_idx)+'.encoder_attn.v_proj.bias']
                                )
    ...
    attn_probs = nn.functional.dropout(attn_weights, p=config.attention_dropout, training=is_train)
    ...
    if attention_type == 'encoder':
        attn_output = F.linear(attn_output, 
                                fast_weights['model.encoder.layers.'+str(layer_idx)+'.self_attn.out_proj.weight'], 
                                fast_weights['model.encoder.layers.'+str(layer_idx)+'.self_attn.out_proj.bias']
                            )
    elif attention_type == 'decoder':
        attn_output = F.linear(attn_output,
                                fast_weights['model.decoder.layers.'+str(layer_idx)+'.self_attn.out_proj.weight'],
                                fast_weights['model.decoder.layers.'+str(layer_idx)+'.self_attn.out_proj.bias']
                            )
    elif attention_type == 'cross':
        attn_output = F.linear(attn_output,
                                fast_weights['model.decoder.layers.'+str(layer_idx)+'.encoder_attn.out_proj.weight'],
                                fast_weights['model.decoder.layers.'+str(layer_idx)+'.encoder_attn.out_proj.bias']
                            )
...
def functional_bart_encoder_layer(fast_weights, config, layer_idx, hidden_states, attention_mask, layer_head_mask, is_train=True):
    ...
    hidden_states, _, _ = functional_bart_attention(fast_weights, config, hidden_states=hidden_states, layer_idx=layer_idx, attention_type='encoder',attention_mask=attention_mask, layer_head_mask=layer_head_mask, is_train=is_train)
    hidden_states = nn.functional.dropout(hidden_states, p=config.dropout, training=is_train)
    hidden_states = residual + hidden_states
    hidden_states = F.layer_norm(hidden_states, [config.hidden_size], 
                              weight=fast_weights['model.encoder.layers.'+str(layer_idx)+'.self_attn_layer_norm.weight'],
                              bias=fast_weights['model.encoder.layers.'+str(layer_idx)+'.self_attn_layer_norm.bias'])
    residual = hidden_states
    hidden_states = F.linear(hidden_states, fast_weights['model.encoder.layers.'+str(layer_idx)+'.fc1.weight'], fast_weights['model.encoder.layers.'+str(layer_idx)+'.fc1.bias'])
    hidden_states = F.gelu(hidden_states)
    hidden_states = nn.functional.dropout(hidden_states, p=config.activation_dropout, training=is_train)
    hidden_states = F.linear(hidden_states, fast_weights['model.encoder.layers.'+str(layer_idx)+'.fc2.weight'], fast_weights['model.encoder.layers.'+str(layer_idx)+'.fc2.bias'])
    hidden_states = nn.functional.dropout(hidden_states, p=config.dropout, training=is_train)
    hidden_states = residual + hidden_states
    hidden_states = F.layer_norm(hidden_states, [config.hidden_size], 
                              weight=fast_weights['model.encoder.layers.'+str(layer_idx)+'.final_layer_norm.weight'],
...
def functional_bart_decoder_layer(fast_weights, config, layer_idx, hidden_states, attention_mask=None, encoder_hidden_states=None, \
    ...
    hidden_states = F.layer_norm(hidden_states, [config.d_model],\
        weight=fast_weights['model.decoder.layers.'+str(layer_idx)+'.self_attn_layer_norm.weight'],
    ...
    if encoder_hidden_states is not None:
        ...
        hidden_states = residual + hidden_states
        hidden_states = F.layer_norm(hidden_states, [config.d_model],\
            weight=fast_weights['model.decoder.layers.'+str(layer_idx)+'.encoder_attn_layer_norm.weight'],
            bias=fast_weights['model.decoder.layers.'+str(layer_idx)+'.encoder_attn_layer_norm.bias']
        )
    ...
    # Fully Connected
    residual = hidden_states
    hidden_states = F.gelu(F.linear(hidden_states, fast_weights['model.decoder.layers.'+str(layer_idx)+'.fc1.weight'], fast_weights['model.decoder.layers.'+str(layer_idx)+'.fc1.bias']))
    hidden_states = nn.functional.dropout(hidden_states, p=config.activation_dropout, training=is_train)
    hidden_states = F.linear(hidden_states, fast_weights['model.decoder.layers.'+str(layer_idx)+'.fc2.weight'], fast_weights['model.decoder.layers.'+str(layer_idx)+'.fc2.bias'])
    hidden_states = nn.functional.dropout(hidden_states, p=config.dropout, training=is_train)
    hidden_states = residual + hidden_states
    hidden_states = F.layer_norm(hidden_states, [config.d_model],\
        weight=fast_weights['model.decoder.layers.'+str(layer_idx)+'.final_layer_norm.weight'],
        bias=fast_weights['model.decoder.layers.'+str(layer_idx)+'.final_layer_norm.bias']
    )
...
def functional_bart_encoder(fast_weights, config, input_ids=None, attention_mask=None, is_train=True):
    ...
    inputs_embeds = F.embedding(input_ids, weight=fast_weights['model.shared.weight']) * embed_scale

    embed_pos = functional_bart_positional_embedding(fast_weights, config, input, is_decoder=False)
    embed_pos = embed_pos.to(inputs_embeds.device)

    hidden_states = inputs_embeds + embed_pos

    hidden_states = F.layer_norm(hidden_states, [embed_dim], fast_weights['model.encoder.layernorm_embedding.weight'], fast_weights['model.encoder.layernorm_embedding.bias'])
    hidden_states = nn.functional.dropout(hidden_states, p=dropout, training=is_train)
...
def functional_bart_decoder(fast_weights, config, input_ids=None, attention_mask=None, encoder_hidden_states=None, encoder_attention_mask=None,\
    ...
    inputs_embeds = F.embedding(input, weight=fast_weights['model.shared.weight']) * embed_scale
    ...
    # embed positions
    positions = functional_bart_positional_embedding(fast_weights, config, input, past_key_values_length, is_decoder=True)
    positions = positions.to(inputs_embeds.device)
    
    hidden_states = inputs_embeds + positions
    hidden_states = F.layer_norm(hidden_states, [embed_dim], weight=fast_weights['model.decoder.layernorm_embedding.weight'], bias=fast_weights['model.decoder.layernorm_embedding.bias'])
...
if __name__ == '__main__':
    ...
    print(functional_bart(fast_weights, model.config, input_ids=input_ids, attention_mask=attention_mask, is_train=False)[0])