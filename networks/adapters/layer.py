...
class AdapterLayer(AdapterLayerBase):
    ...
    def adapter_stack(self, adapter_setup: Stack, hidden_states, input_tensor, layer_type, layer_norm, lvl=0):
        """
        ...
        for i, adapter_stack_layer in enumerate(adapter_setup):
            ...
            # Case 5: We have a single adapter which is part of this module -> forward pass
                ...
                hidden_states, _, up = adapter_layer(hidden_states, residual_input=residual, layer_type=layer_type)
    ...
    def adapter_layer_forward(self, hidden_states, input_tensor, layer_type, layer_norm):
        """
        ...
        if not skip_adapters and (len(set(self.adapters.keys()) & adapter_setup.flatten()) > 0):
            ...
            if isinstance(adapter_setup, Stack):
                hidden_states, _, input_tensor = self.adapter_stack(
                    adapter_setup, hidden_states, input_tensor, layer_type, layer_norm
                )
    ...
    def forward(self, hidden_states, input_tensor, layer_type, layer_norm):
        return self.adapter_layer_forward(hidden_states, input_tensor, layer_type, layer_norm)
